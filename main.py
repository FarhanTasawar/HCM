...
import calendar, base64, csv, os, io
...
<div class="toolbar">
  ...
  <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
      <a href="{build_export_query('employees','csv')}" class="btn btn-gray btn-sm">📝 Export CSV</a>
      <a href="{build_export_query('employees','xlsx')}" class="btn btn-primary btn-sm">📄 Export Excel</a>
    </div>
...
<div class="toolbar" style="justify-content:space-between;">
  ...
  <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
    <a href="{build_export_query('attendance','csv', {'year':year, 'month':month})}" class="btn btn-gray btn-sm">📝 Export CSV</a>
    <a href="{build_export_query('attendance','xlsx', {'year':year, 'month':month})}" class="btn btn-primary btn-sm">📄 Export Excel</a>
  </div>
</div>
<div class="toolbar">
...
<div class="toolbar">
  ...
  <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
    <a href="{build_export_query('payroll','csv', {'year':year, 'month':month})}" class="btn btn-gray btn-sm">📝 Export CSV</a>
    <a href="{build_export_query('payroll','xlsx', {'year':year, 'month':month})}" class="btn btn-primary btn-sm">📄 Export Excel</a>
  </div>
</div>
<div class="card">
  <div class="card-header">
...
<div class="toolbar" style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;justify-content:space-between;">
  <div>{tabs}</div>
  <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
    <a href="{build_export_query('leave_requests','csv', {'status':status_f})}" class="btn btn-gray btn-sm">📝 Export CSV</a>
    <a href="{build_export_query('leave_requests','xlsx', {'status':status_f})}" class="btn btn-primary btn-sm">📄 Export Excel</a>
  </div>
</div>
<div class="card">
...
<div class="toolbar">
  ...
  <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
    <a href="{build_export_query('attendance','csv', {'emp_id':eid if eid!='all' else 'all', 'year':year, 'month':month})}" class="btn btn-gray btn-sm">📝 Export CSV</a>
    <a href="{build_export_query('attendance','xlsx', {'emp_id':eid if eid!='all' else 'all', 'year':year, 'month':month})}" class="btn btn-primary btn-sm">📄 Export Excel</a>
...
@admin_only
def export_data():
    """Export dataset as CSV or XLSX for the admin interface."""
    dataset = request.args.get("dataset","employees")
    ...
    if dataset == "employees":
        ...
        columns = ["employee_id","name","designation","department",
                   "organization","salary","joining_date","phone",
                   "email","address","duty_start","duty_end",
                   "active","resigned_at","created_at"]
    elif dataset == "attendance":
        rows = db.get_all_attendance_month(year, month)
        if emp_id != "all":
            rows = [r for r in rows if r["employee_id"] == emp_id]
        columns = ["att_date","employee_id","emp_name","designation",
                   "department","check_in","check_out","status",
                   "late_minutes","manual","note","location_in",
                   "location_out","created_at"]
    elif dataset == "payroll":
        rows = db.get_all_payroll_month(year, month)
        columns = ["employee_id","employee_name","designation","department",
                   "organization","year","month","working_days",
                   "present_days","absent_days","leave_days","half_days",
                   "late_count","late_minutes","gross_salary",
                   "absent_deduct","late_deduct","bonus","total_deduct",
                   "net_salary","generated_at"]
    elif dataset == "leave_requests":
        rows = db.get_leave_requests(
            employee_id=None if emp_id == "all" else emp_id,
            status=None if status == "all" else status
        )
        columns = ["employee_id","emp_name","leave_type","from_date",
                   "to_date","days","reason","status","admin_note",
                   "created_at"]
    else:
        return "Invalid export dataset.", 400

    return generate_export_response(rows, columns, dataset, fmt)


def build_export_query(dataset: str, fmt: str, extra: dict = None) -> str:
    """Build a safe URL for exporting a dataset in the requested format."""
    params = {"dataset": dataset, "format": fmt}
    if extra:
        for k, v in extra.items():
            if v is not None:
                params[k] = v
    query = "&".join(f"{quote_plus(str(k))}={quote_plus(str(v))}" for k, v in params.items())
    return f"/export?{query}"


def rows_to_csv(rows, columns):
    """Convert row dictionaries into UTF-8 CSV bytes."""
    output = io.StringIO()
...
def generate_export_response(rows, columns, dataset, fmt):
    """Build an HTTP download response for exported data."""
    filename = f"{dataset.replace('_','-')}-{datetime.now().strftime('%Y%m%d-%H%M%S')}.{fmt}"
    if fmt == "csv":
        data = rows_to_csv(rows, columns)
        resp = make_response(data)
        resp.headers["Content-Type"] = "text/csv; charset=utf-8"
    else:
        data = rows_to_xlsx(rows, columns)
        resp = make_response(data)
